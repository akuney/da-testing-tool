select
	...
	ifnull(replace(substr(old_settings from (locate('$',old_settings) + 1)), '',', ''')+ 0.0, 0.0) as `Old Amount`,
	ifnull(replace(substr(new_settings from (locate('$',new_settings) + 1)), ',', '') + 0.0, 0.0) as `New Amount`,
