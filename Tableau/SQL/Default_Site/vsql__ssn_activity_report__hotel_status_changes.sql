...
select
	users.*,
	(case
	  when users."User First Auction Participation" = status_changes."First Auction Participation"
	  then 1
	  else 0
  end) as "First Advertiser for User",
	(case
	  when users."User First Auction Participation" = hotel_property_status_changes."First Auction Participation"
	  then 1
	  else 0
	...
	(case
		...
		when "Advertising Status" = 'Suspended' and "Previous Advertising Status" like '%Active%' then 'Suspended'
...
left join
	...
	(select
		...
		(case
			...
			when pas.advertiser_id is null and budgets.effective_budget >= 0.25 then 
				(case
					when sold.max_change_date >= sold.max_participation_date and sold.suspended_for_non_payment = 1 then 'Suspended' 
					when sold.max_change_date >= sold.max_participation_date and sold.suspended_for_non_payment < 1 then 'Manually Paused'
		...
		(case
			...
			when previous_pas.advertiser_id is null and previous_budgets.effective_budget >= 0.25 then 
				(case 
					when sold.previous_max_change_date >= sold.previous_max_participation_date and sold.suspended_for_non_payment = 1 then 'Suspended'
					when sold.previous_max_change_date >= sold.previous_max_participation_date and sold.suspended_for_non_payment < 1 then 'Manually Paused'
					else 'Paused No Traffic'
				end)
	...
	from
		(select
			...
			max(dates_sold_hotels_changes.suspended_for_non_payment) as suspended_for_non_payment,
		...
		from
			(select
				...
				max(dates_sold_hotels.suspended_for_non_payment) as suspended_for_non_payment,
