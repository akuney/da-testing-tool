    ...
    sql_string = """ 
        ...
        CREATE TABLE Intent_media_sandbox_production.EA_ppa_orbitz_channel_1 AS
               ...
               SELECT case WHEN publisher_price -  Min(advertiser_price) OVER (Partition BY a.request_ID) <= 1 AND
                ...
                and i.ip_address_blacklisted = false  AND a.requested_at_date_in_et = to_date('""" + strmonth + strday + stryear + """','mm/dd/yyyy') 
...
def do_analysis2_advertisers(stryear, strmonth, strday):
    ...
    sql_string = """ 
      ...
      DROP TABLE IF EXISTS  Intent_media_sandbox_production.EA_ppa_competition_advertiser_stage_2;
      CREATE TABLE Intent_media_sandbox_production.EA_ppa_competition_advertiser_stage_2
      AS
      ...
      FROM 
            ...
            when e2.ad_display_name = 'Booking.com' then 'Priceline'                    
                ...
                CASE WHEN ad.id in (131,130,89,187,184,173,129,206,207) THEN 'US' 
                   WHEN ad.id in (216,147,143,215) THEN 'UK'
                   ELSE 'Unknown' END AS Publisher_Region,
      ...
      GRANT ALL on Intent_media_sandbox_production.EA_ppa_competition_advertiser_stage_2 to PUBLIC;
...
def do_analysis3(stryear, strmonth, strday, version):
    ...
    sql_string = """ 
      ...
      DROP TABLE IF EXISTS  Intent_media_sandbox_production.EA_ppa_competition_""" + version + """stage_3;
      CREATE TABLE Intent_media_sandbox_production.EA_ppa_competition_""" + version + """stage_3
      AS

      SELECT b.*, ru.runner_up
      FROM
      (SELECT a.*, 1 as meta_rank FROM Intent_media_sandbox_production.EA_ppa_competition_""" + version + """stage_2 a
      ...
      FROM Intent_media_sandbox_production.EA_ppa_competition_""" + version + """stage_2) b
      LEFT OUTER JOIN 
      ...
      GRANT ALL on Intent_media_sandbox_production.EA_ppa_competition_""" + version + """stage_3 to PUBLIC;
...
def do_analysis5(stryear, strmonth, strday, version):
    ...
    sql_string = """ 
      ...
      DROP TABLE IF EXISTS  Intent_media_sandbox_production.EA_ppa_orbitz_channel_all_"""  + strmonth + strday + stryear + """;
      CREATE TABLE Intent_media_sandbox_production.EA_ppa_orbitz_channel_all_"""  + strmonth + strday + stryear + """
      AS
        ...
        SELECT  requested_at_date_in_et,
          ...
          FROM Intent_media_sandbox_production.EA_ppa_orbitz_channel_4;
                   ...
                   GRANT ALL on Intent_media_sandbox_production.EA_ppa_orbitz_channel_all_"""  + strmonth + strday + stryear + """ to PUBLIC
...
# _"""  + strmonth + strday + stryear + """
    ...
    sql_string = """ 
          ...
          DROP TABLE IF EXISTS Intent_media_sandbox_production.EA_ppa_auction_position_"""  + strmonth + strday + stryear + """;
          CREATE TABLE Intent_media_sandbox_production.EA_ppa_auction_position_"""  + strmonth + strday + stryear + """
          AS                
                ...
                when e2.ad_display_name = 'Booking.com' then 'Priceline'                    
                    ...
                    ---normalizes display names between Publishers and Advertisers which are different entity_IDs for teh same company.  Removes capitals and .com
                    CONCAT(LEFT(s.display_name,1) , LOWER(Right(s.display_name, length(s.display_name) -1))) as site, 
                    ad.name as "Ad Unit",  a.ad_unit_id, lp.page_type,  a.requested_at_date_in_et, a.site_currency,  a.rank_in_page, 
                    revenue, clicks, a.advertiser_id, Dom_Intl, auction_position,
                    REPLACE(CONCAT(LEFT( e1.ad_display_name,1), LOWER(Right(e1.ad_display_name, length( e1.ad_display_name) -1))),'.com','') as Advertiser, 
                     Publisher_price, advertiser_price,
          ...
          GRANT ALL on Intent_media_sandbox_production.EA_ppa_auction_position_"""  + strmonth + strday + stryear + """ to PUBLIC;
