 ...
 FROM (
       ...
       (
                 select 
                a.publisher_user_id,
                min(a.requested_at_date_in_et) as first_login_date,                
                0 as session_id,
                traffic_share_type,
                SUM(c.actual_cpc) as revenue
        from intent_media_log_data_production.ad_calls a
        JOIN  intent_media_log_data_production.clicks c ON a.request_id = c.ad_call_request_id 
        where ad_unit_type = 'META' 
                and a.ip_address_blacklisted = false
                and c.ip_address_blacklisted = false  
                and a.site_type = 'ORBITZ_GLOBAL'
                and c.site_type = 'ORBITZ_GLOBAL'
                and fraudulent = false
                AND a.publisher_user_id not in 
        ...
        group by a.publisher_user_id, a.traffic_share_type
        ) a1
        left join 
        (
                select
