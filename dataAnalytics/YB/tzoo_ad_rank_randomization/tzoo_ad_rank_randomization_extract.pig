...
--                       ip_address_blacklisted == 0 AND
--                       (advertiser_id == 59528 OR advertiser_id == 59777 OR advertiser_id == 61224 OR advertiser_id == 87697 OR advertiser_id == 106574);
...
--joined_ac_i = JOIN impressions BY request_id, ad_calls BY request_id;
...
--
--filtered_ac_i_c = FILTER joined_ac_i_c BY HoursBetween(ac_i::ac_timestamp, clicks::requested_at) <= 24;
--
--ac_i_c = FOREACH filtered_ac_i_c GENERATE
--         ac_i::ac_request_id,
...
STORE conversions_deduped INTO '$STORE_PATH' USING PigStorage('\t');
