...
/* Table 2 - Ad Calls with hotel Property ID quartile */   
...
create table intent_media_sandbox_production.SP_PPA_auction_vpc_ac_m as
    select
     ac.*,
     m.quartile as quartile
    from intent_media_sandbox_production.SP_PPA_auction_vpc_ac ac
      left join intent_media_sandbox_production.SP_PPA_auction_property_c_bins m /* ...property_ac_bins (property_c_bins) if ranked by ad_call (click) volume  ...*/
    on ac.hotel_property_id = m.hotel_property_id
;
