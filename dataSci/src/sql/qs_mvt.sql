...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_ac
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_ac AS
SELECT request_id,
	requested_at,
	publisher_user_id,
	site_type,
...
FROM intent_media_log_data_production.ad_calls
        -- Only looking at a day; MVT started mid-day 5/29; day 0: 5/30
WHERE requested_at_date_in_et between '2014-05-30' and '2014-05-30'
	AND ip_address_blacklisted = 0
	-- TODO: Add all sites on which we are running the MVT?
	-- TODO: Decide if looking at this list is enough
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_c_ac
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_c_ac AS
...
FROM intent_media_log_data_production.clicks
        -- Looking at clicks for the day and day after; clicks can be attributed within 24 hr
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_c_raw
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_c_raw AS
SELECT request_id,
	requested_at,
	ad_call_request_id,
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_attribution_i
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_attribution_i AS
SELECT external_id,
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_c_complete
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_c_complete AS
SELECT cl.request_id,
	...
	i.advertiser_id,
	cl.actual_cpc
FROM intent_media_sandbox_production.SPDP_MVT_55_c_raw cl
INNER JOIN intent_media_sandbox_production.SPDP_MVT_55_attribution_i i ON (cl.external_impression_id = i.external_id)
;

DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_con
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_con AS
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_con_cl
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_con_cl AS
...
FROM intent_media_sandbox_production.SPDP_MVT_55_con con
INNER JOIN intent_media_sandbox_production.SPDP_MVT_55_c_complete cl ON (con.webuser_id = cl.webuser_id AND con.entity_id = cl.advertiser_id)
;

DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_cl_con
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_cl_con AS
SELECT cl.request_id,
	cl.requested_at,
	cl.ad_call_request_id,
	cl.actual_cpc,
	CASE WHEN att_con.conversions is null THEN 0 ELSE att_con.conversions END AS clicked_conversions
FROM intent_media_sandbox_production.SPDP_MVT_55_c_complete cl
LEFT JOIN (

SELECT con_cl.cl_request_id,
	count(con_cl.con_request_id) AS conversions
FROM intent_media_sandbox_production.SPDP_MVT_55_con_cl con_cl
INNER JOIN (

SELECT con_request_id,
	min(con_cl_time_diff) AS min_con_cl_time_diff
FROM intent_media_sandbox_production.SPDP_MVT_55_con_cl
...
DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_cl
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_cl AS
SELECT ad_call_request_id,
	count(request_id) AS clicks,
	sum(actual_cpc) AS revenue,
	sum(actual_cpc*actual_cpc) AS revenue_2_c,
...
FROM intent_media_sandbox_production.SPDP_MVT_55_cl_con
GROUP BY ad_call_request_id
;

DROP TABLE IF EXISTS intent_media_sandbox_production.SPDP_MVT_55_final
;

CREATE TABLE intent_media_sandbox_production.SPDP_MVT_55_final AS
SELECT mvt_value_1,
...
FROM intent_media_sandbox_production.SPDP_MVT_55_ac ac
LEFT JOIN intent_media_sandbox_production.SPDP_MVT_55_cl c ON (ac.request_id = c.ad_call_request_id)
...
SELECT * FROM intent_media_sandbox_production.SPDP_MVT_55_final
