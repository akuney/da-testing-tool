...
/* Table 2 - Ad Calls with Market ID decile */   
...
create table intent_media_sandbox_production.SP_PPA_auction_vpc_ac_m as
    select
     ac.*,
     m.decile as decile_by_market_search_volume
    from intent_media_sandbox_production.SP_PPA_auction_vpc_ac ac
      left join intent_media_sandbox_production.SP_PPA_auction_market_ac_bins m
    on ac.market_id = m.market_id
;
...
create table intent_media_sandbox_production.SP_PPA_auction_vpc_final as
    select 
        site_type,
        advertiser_id,
        decile_by_market_search_volume,
