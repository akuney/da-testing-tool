...
-- max cumulative pause count for each advertiser
-- for each month
DROP TABLE IF EXISTS intent_media_sandbox_production.sp_adv_attrition_advertiser_entities_months_pauses;
CREATE TABLE intent_media_sandbox_production.sp_adv_attrition_advertiser_entities_months_pauses AS
SELECT
  em1.*,
...
-- month, year, entity features, pause count,
-- market and report segment 
DROP TABLE IF EXISTS intent_media_sandbox_production.sp_adv_attrition_months_pauses_markets_segments;
CREATE TABLE intent_media_sandbox_production.sp_adv_attrition_months_pauses_markets_segments AS
SELECT
  em.id as advertiser_id,
  em.month,
  em.year,
...
FROM intent_media_sandbox_production.sp_adv_attrition_advertiser_entities_months_pauses em
  ...
  LEFT JOIN intent_media_sandbox_production.sp_adv_attrition_pause_time p
    ON em.id = p.advertiser_id AND ((em.month = MONTH(p.campaign_paused_time)-1 AND em.year=YEAR(p.campaign_paused_time))
                                 OR (em.month = MONTH(p.campaign_paused_time)+11 AND em.year=YEAR(p.campaign_paused_time)-1));


-- bring in performance features
SELECT
    *
FROM
    intent_media_sandbox_production.sp_adv_attrition_months_pauses_markets_segments f
    LEFT JOIN intent_media_sandbox_production.sp_adv_attrition_advertiser_performance_comp_prior_month p
        ON f.advertiser_id = p.advertiser_id
        AND f.year = p.year
        AND f.month = p.month;
