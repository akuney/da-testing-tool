...
/* Table 2 - Ad Calls with hotel brand decile by click */   
drop table if exists intent_media_sandbox_production.SP_PPA_auction_vpc_ac_m;
create table intent_media_sandbox_production.SP_PPA_auction_vpc_ac_m as
    select
     ac.*,
     b.decile as decile
    from 
      ...
      from
      intent_media_sandbox_production.SP_PPA_auction_vpc_ac a
      left join
      intent_media_production.hotel_properties b
      on 
      a.hotel_property_id = b.id
      ) ac
      left join 
      intent_media_sandbox_production.SP_PPA_auction_brand_c_bins b /* Exclude the brand_id mapped to UNKNOWN */
    on ac.brand_id = b.brand_id
;

 /* Table 3a - Advertiser Ids from Impressions */   
...
/* Table 3b - Ad Calls with Advertiser Id from Impressions */   
...
create table intent_media_sandbox_production.SP_PPA_auction_vpc_ac_i as
    select
     ...
     i.external_id,
     i.advertiser_id 
    from intent_media_sandbox_production.SP_PPA_auction_vpc_ac_m ac
