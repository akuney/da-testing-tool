drop table if exists intent_media_sandbox_production.SP_PPA_auction_property_c_count;
create table intent_media_sandbox_production.SP_PPA_auction_property_c_count as
select
hotel_property_id,
click_count,
        ...
        from 
                ...
                from
	                ...
	                and site_type in ('CHEAPTICKETS', 'EBOOKERS',  'ORBITZ_GLOBAL')
                        ...
                        and hotel_property_id is not null) ac
                        left join
                        (select *
                        from
          ...
          group by 1) click_count_by_property      
order by rank asc            
;

drop table if exists intent_media_sandbox_production.SP_PPA_auction_property_c_bins;
create table intent_media_sandbox_production.SP_PPA_auction_property_c_bins as
select 
hotel_property_id,
rank,
...
from
  (select 
        ...
        max(t1.rank) as rank
        from 
        intent_media_sandbox_production.SP_PPA_auction_property_c_count t1
        inner join 
        intent_media_sandbox_production.SP_PPA_auction_property_c_count t2
        on t2.rank <= t1.rank
